...
- **`AchievementUnlock` records unlock _events_, not counts (#91).** `AchievementSnapshot` stores a cumulative `unlockedCount` per day, which only yields a year total via `max − min` across snapshots — useless with ≤1 snapshot in a year (the common case, since onboarding seeds no achievement baseline). `AchievementUnlock` instead stores one row per unlocked achievement keyed by its real `unlockedAt` (from Steam's `unlocktime`, unix SECONDS × 1000), so Year-in-Review counts by UTC year directly and is correct on day one. `unlocktime 0` ("time unknown") is excluded at write time. The compound PK `(steamId, appId, apiName)` makes re-recording idempotent; `@@index([steamId, unlockedAt])` serves the per-user year query. Written by the nightly snapshot job (via the cached achievement repository, so the cumulative-count pass's fetches are reused) and seeded by the onboarding backfill so prior years populate retroactively. **Population is budgeted/rotational (theme-5 T1):** each nightly run covers the hot set (top-20 by two-week playtime) plus one day-keyed rotation window of ≤ `ACHIEVEMENT_UNLOCK_NIGHTLY_LIMIT` (40) remaining games, so table completeness is *eventual* — full coverage within `ceil(R/40)` nights, no schema change. Rows carry the real `unlockedAt`, so delayed recording writes identical rows (delayed, never lost). Readers must expect recently-unavailable rows for cold games during the convergence window.
...
- **The snapshot composite indexes are now actually exercised (ERR-0023).** `@@index([steamId, date])` on `PlaytimeSnapshot` and `@@index([steamId, unlockedAt])` on `AchievementUnlock` existed from the start, but the insights/history read paths issued steamId-only scans that never let them prune. Theme 1 bounded every reader (year window + separate pre-year baseline fetch for Year-in-Review, `unlockedAt` window for unlocks, rendered-window `since` for `/history`, DB-side `distinct` for review years) — **with zero schema change**: no new index or migration was needed (the DATA-7 finding dissolved once the queries passed bounds).
