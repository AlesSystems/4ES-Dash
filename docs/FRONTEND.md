...
5. **History window semantics (ERR-0023).** `/history` renders a bounded window — 53 weeks (`?bucket=week`) or 25 months (`?bucket=month`), constants in `lib/history/aggregate.ts` (`HISTORY_LOOKBACK`), with the fetch's `since` **floored to the bucket boundary** (`historyWindowStart`: ISO week start / month start) so the oldest bar is never under-counted. Empty-state copy must distinguish **"no data ever"** ("No history yet") from **"no data in window"** (the "No recent playtime" quiet state) — a user whose snapshots all predate the window has history and must never be told otherwise. The idle insights lookback (365 d, `IDLE_LOOKBACK_DAYS`) is bug-3's shipped surface.
6. **Big lists crossing an RSC→client boundary: project + slice server-side, page via URL state.** The canonical recipe is `/library`: the RSC sorts/filters on the full server-side shape, then ships **only the visible page** of a client-safe projection (`LibraryTileGame` via `toLibraryTile` in `lib/games/sort.ts` — exactly the fields the tiles render, nothing else serializes). Pagination is the `?limit=` search param (`parseLimitParam`: invalid → 24, clamps to 960, snaps down to a multiple of `PAGE_SIZE = 24`), so `components/library/LibraryResults.tsx` is an RSC and the only client code is the `LoadMoreButton` leaf (`router.replace` with `limit + PAGE_SIZE`, `{ scroll: false }`). Counts stay honest: the results copy's "of Y" is the **filtered** count passed as `filteredTotal` (page `shown.length` computed pre-slice), never the all-games total. Filter/sort/search/multiplayer changes drop `limit` in `LibraryControls.updateUrl` (set-changing keys reset paging); a `view` toggle preserves it (markup-only change). Never ship an unbounded array across a `'use client'` boundary.
7. **Shell streaming (ERR-0021).** Any async RSC mounted in a layout above `{children}` MUST sit behind its own geometry-matched `<Suspense>` boundary — an un-suspended async component in the shell gates document flush of *every* route on its I/O (the root-layout header/sidebar once put 3 limiter-serialized Steam calls on every first paint). Canonical patterns: the shell boundaries in `app/layout.tsx` (`HeaderSkeleton`/`SidebarSkeleton`) and the per-section boundaries on `/game/[appId]`. A Suspense *fallback* must never render an async server component (the fallback itself would suspend and silently reinstate the coupling) — placeholders are static or `"use client"` only.
8. **Explicit duration budgets on job-triggering pages (ERR-0024).** `/onboarding` carries `export const maxDuration = 60`, mirroring `app/settings/page.tsx` — the fan-out itself is bounded in code (`ONBOARDING_UNLOCK_LIMIT` in `server/jobs/onboarding-backfill.ts`), the export is only a non-load-bearing ceiling (60s = Vercel Hobby max).
