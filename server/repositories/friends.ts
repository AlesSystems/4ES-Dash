...
/**
 * Fetches the configured user's friend list, enriches each entry with full
...
export async function getFriends(): Promise<FriendsResult> {
  ...
  const friendListResult = await cache(cacheKey('friend-list', STEAM_ID), TTL.friendList, () =>
    getFriendList(STEAM_ID),
  );
  ...
  const summariesResult = await cache(
    cacheKey('friends-summaries', STEAM_ID),
    TTL.playerSummaries,
