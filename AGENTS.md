...
This repo ships two project-specific Codex skills in `.Codex/skills/`:
...
- [docs/HOOKS.md](docs/HOOKS.md) — Codex hooks (`.Codex/settings.json`) and how to disable them locally
- [docs/MCP.md](docs/MCP.md) — MCP servers wired in `.mcp.json` (GitHub, Context7) and deferred ones
- [docs/design/](docs/design/README.md) — Codex Design handoff bundle: HTML/JSX page mockups (Dashboard, Library, Game Detail, Friends, Settings, Year in Review) + design chat transcripts. Reference (read transcripts first, recreate pixel-perfect) when implementing each page in Phases 1+. Prototypes, not source — excluded from build/lint/test.
...
Multiple agents may work on different modules simultaneously. Do not revert unexpected changes — another agent may have made them. When delegating complex tasks, spawn sub-agents in parallel using Codex-sonnet-4-6 by default.
